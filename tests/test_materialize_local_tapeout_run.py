...
def test_materialize_local_tapeout_run_success_and_smoke_compatible(tmp_path: Path) -> None:
    ...
    assert route_rows[0]["width_um"] == 0.5
    assert route_rows[0]["enclosure_um"] == 1.5
    assert route_rows[0]["bends"][0]["radius_um"] == 10.0
    assert route_rows[0]["coupling_coeff"] == 0.01
    assert route_rows[1]["coupling_coeff"] == 0.02
...
def test_materialize_local_tapeout_run_requires_force_for_conflicting_files(tmp_path: Path) -> None:
    ...
    assert repaired_rows[0]["route_id"] == "e1:gc_in.out->wg_1.in"
