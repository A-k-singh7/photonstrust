...
def test_phase54_satellite_canonical_configs_validate() -> None:
    root = Path(__file__).resolve().parents[1]
    canonical_dir = root / "configs" / "canonical"
    paths = sorted(canonical_dir.glob("phase54_satellite_*.yml"))
    assert paths, "Expected Phase 54 satellite canonical configs"
...
def test_phase54_satellite_canonical_baselines_match_fixture() -> None:
    root = Path(__file__).resolve().parents[1]
    fixture = root / "tests" / "fixtures" / "canonical_phase54_satellite_baselines.json"
    assert fixture.exists(), "Missing baseline fixture. Run scripts/generate_phase54_satellite_canonical_baselines.py"
    ...
    for entry in configs:
        ...
        for scenario in scenarios:
            ...
            obs_bg = [float(r.background_counts_cps) for r in results]
            ...
            for obs, exp in zip(obs_qber, expected["qber"]):
                assert obs == pytest.approx(float(exp), rel=1e-6, abs=1e-12)
            for obs, exp in zip(obs_bg, expected.get("background_counts_cps") or [0.0] * len(obs_bg)):
                assert obs == pytest.approx(float(exp), rel=1e-6, abs=1e-12)
