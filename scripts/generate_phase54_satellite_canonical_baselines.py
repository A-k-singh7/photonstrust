"""Generate baseline fixtures for Phase 54 canonical satellite presets."""
...
def main() -> None:
    ...
    output_path = root / "tests" / "fixtures" / "canonical_phase54_satellite_baselines.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    config_paths = sorted(canonical_dir.glob("phase54_satellite_*.yml"), key=lambda p: p.name.lower())
    if not config_paths:
        raise SystemExit(f"No canonical Phase 54 satellite configs found under {canonical_dir}")
    ...
    payload = {
        ...
        "kind": "photonstrust.canonical_baselines.phase54_satellite",
        "configs": out_configs,
    }
