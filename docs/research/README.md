# PhotonTrust Research Documentation

Start here:
- `00_overview.md`

Core research set:
- `01_market_landscape.md`
- `02_architecture_and_interfaces.md`
- `03_physics_models.md`
- `04_network_kernel_and_protocols.md`
- `05_calibration_and_uncertainty.md`
- `06_optimization_and_decisions.md`
- `07_reliability_card_standard.md`
- `08_benchmarks_and_datasets.md`
- `09_product_and_ux.md`
- `10_roadmap_and_milestones.md`
- `11_risks_quality_and_governance.md`
- `12_web_research_update_2026-02-12.md`
- `13_business_expansion_and_build_plan_2026-02-12.md`
- `14_physics_core_open_science_master_plan_2026-02-12.md`
- `15_platform_rollout_plan_2026-02-13.md`
- `16_web_research_update_2026-02-13.md`

Deep-dive set:
- `deep_dive/00_deep_dive_index.md`

Upgrade ideas (consolidated maps):
- `../upgrades/README.md`

Program operations docs:
- `deep_dive/12_execution_program_24_weeks.md`
- `deep_dive/13_raci_matrix.md`
- `deep_dive/14_milestone_acceptance_templates.md`
- `deep_dive/15_research_to_build_protocol.md`
- `../operations/README.md`

Templates:
- `../templates/research_brief_template.md`
- `../templates/implementation_plan_template.md`
- `../templates/validation_report_template.md`
